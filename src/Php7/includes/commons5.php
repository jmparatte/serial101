...
$SCRIPT_PATH = substr($SCRIPT_NAME,0,strlen($SCRIPT_NAME)-strlen($SCRIPT_FILE));
if( substr(REQUEST_URI,0,strlen($SCRIPT_NAME))!=$SCRIPT_NAME) $SCRIPT_NAME = $SCRIPT_PATH;
define( 'SCRIPT_NAME',	$SCRIPT_NAME );
define( 'SCRIPT_PATH',	$SCRIPT_PATH );
define( 'SCRIPT_FILE',	$SCRIPT_FILE );
define( 'SCRIPT_ONLY',	basename(basename(SCRIPT_FILE,'.html'),'.php') );
define( 'SCRIPT_FILE1',	substr($_SERVER['SCRIPT_NAME'], strlen($SCRIPT_PATH)) );
...
define( 'XON',	"\x11" );	// 11	XON=DC1=Ctrl-Q Pause transmission
define( 'XOFF',	"\x13" );	// 13	XOFF=DC3=Ctrl-S Resume transmission
...
define( 'PILCROW','&para;'); // B6	¶ Pied de mouche
define( 'MIDDOT','&middot;'); // B7	• Point médian
define( 'BR',	'<br />' );	// HTML Break line
define( 'HR',	'<hr />' );	// HTML Horizontal line
...
function nf(&$var,$f=FALSE){return $var === NULL ? $f : $var;}
function fn(&$var,$n=NULL){return $var === FALSE ? $n : $var;}

function ez(&$e,$z=0){return $e === '' ? $z : $e;}
function ze(&$z,$e=''){return $z === 0 ? $e : $z;}

//------------------------------------------------------------------------------

function an(&$a,$n=NULL){return isset($a) ? $a : $n;}
function ain(&$a,$i,$n=NULL){return isset($a[$i]) ? $a[$i] : $n;}
function aiin(&$a,$i,$j,$n=NULL){return isset($a[$i][$j]) ? $a[$i][$j] : $n;}
function aiiin(&$a,$i,$j,$k,$n=NULL){return isset($a[$i][$j][$k]) ? $a[$i][$j][$k] : $n;}
function aiiiin(&$a,$i,$j,$k,$l,$n=NULL){return isset($a[$i][$j][$k][$l]) ? $a[$i][$j][$k][$l] : $n;}

function ae(&$a,$e=''){return isset($a) ? $a : $e;}
function aie(&$a,$i,$e=''){return isset($a[$i]) ? $a[$i] : $e;}
function aiie(&$a,$i,$j,$e=''){return isset($a[$i][$j]) ? $a[$i][$j] : $e;}
function aiiie(&$a,$i,$j,$k,$e=''){return isset($a[$i][$j][$k]) ? $a[$i][$j][$k] : $e;}
function aiiiie(&$a,$i,$j,$k,$l,$e=''){return isset($a[$i][$j][$k][$l]) ? $a[$i][$j][$k][$l] : $e;}

function az(&$a,$z=0){return isset($a) ? $a : $z;}
function aiz(&$a,$i,$z=0){return isset($a[$i]) ? $a[$i] : $z;}
function aiiz(&$a,$i,$j,$z=0){return isset($a[$i][$j]) ? $a[$i][$j] : $z;}
function aiiiz(&$a,$i,$j,$k,$n=0){return isset($a[$i][$j][$k]) ? $a[$i][$j][$k] : $n;}
function aiiiiz(&$a,$i,$j,$k,$l,$z=0){return isset($a[$i][$j][$k][$l]) ? $a[$i][$j][$k][$l] : $z;}
...
function fan($function,&$a,$n=NULL){return isset($a) ? $function($a) : $n;}
function fain($function,&$a,$i,$n=NULL){return isset($a[$i]) ? $function($a[$i]) : $n;}

function fae($function,&$a,$e=''){return isset($a) ? $function($a) : $e;}
function faie($function,&$a,$i,$e=''){return isset($a[$i]) ? $function($a[$i]) : $e;}

function faz($function,&$a,$z=0){return isset($a) ? $function($a) : $z;}
function faiz($function,&$a,$i,$z=0){return isset($a[$i]) ? $function($a[$i]) : $z;}

//------------------------------------------------------------------------------

function fv($function,$v){return isset($function) ? $function($v) : $v;}
function afv(&$a,$function,$v){return isset($a[$function]) ? $a[$function]($v) : $v;}
function aifv(&$a,$i,$function,$v){return isset($a[$i][$function]) ? $a[$i][$function]($v) : $v;}
...
function uc($str){return strtoupper($str);}
function lc($str){return strtolower($str);}
...
function plain( $text ) { // 2018-01-20
	return htmlspecialchars($text, ENT_NOQUOTES|ENT_HTML5); // 2018-01-16
}
...
//		'HT', 
		...
		preg_replace( 
			'/['.CR.']?['.NL.']/', 
			BR.NL, 
//			htmlspecialchars($text, ENT_HTML5, null, true)
			htmlspecialchars($text, ENT_NOQUOTES|ENT_HTML5) // 2018-01-16
		)
	);
}
...
//2020-05-14: use APPLICATION_JAVASCRIPT rather than TEXT_JAVASCRIPT
	'TEXT_JAVASCRIPT' => [
	...
	'TEXT_CSV_S' => [
		'Content-Type: text/csv; charset=%s',
	],
	'LENGTH' => [
		'Content-Length: %d',
	],
...
//2020-05-14: wrong name, use correct ATTACHEMENT_S name
	'ATTACHEMENT' => [
...
//2020-05-14: application/javascript in replacement of text/javascript
	'APPLICATION_JAVASCRIPT' => [
		'Content-Type: application/javascript; charset=utf-8',
	],
//2020-05-14: application/json
	'APPLICATION_JSON' => [
		'Content-Type: application/json; charset=utf-8',
	],
//2020-05-14: application/jsonp (runnable javascript with callback)
	'APPLICATION_JSONP' => [
		'Content-Type: application/javascript; charset=utf-8',
	],
//2020-05-14: application/xml
	'APPLICATION_XML' => [
...
function echo_content( $contents = '', $headers = [] ) {
	headers_set( $headers );
	echo( $contents );
}

function exit_content( $contents = '', $headers = [], $exit_code = 0 ) {
	echo_content( $contents, $headers );
	exit( $exit_code );
}
...
//2020-01-08:
// - ajout |ENT_SUBSTITUTE dans plain() et html()
// - suppression de la substitution HT dans html()
// - ajout de attr_text()


function attr_text( $text ) { // 2020-01-08
	return htmlspecialchars($text, ENT_COMPAT|ENT_SUBSTITUTE|ENT_HTML5);
}

function plain( $text ) { // 2020-01-08,2018-01-20,2018-01-16
	return htmlspecialchars($text, ENT_NOQUOTES|ENT_SUBSTITUTE|ENT_HTML5); // 2018-01-16
}

function html( $text ) { // 2020-01-08,2018-01-20,2018-01-16
	return preg_replace( 
//		'/['.HT.']/', 
////		'HT', 
//		SP, // 2018-01-16
//		preg_replace( 
			'/['.CR.']?['.NL.']/', 
			BR.NL, 
			htmlspecialchars($text, ENT_NOQUOTES|ENT_SUBSTITUTE|ENT_HTML5) // 2018-01-16
//		)
	);
}
...
//		$html .= (substr($text,0,1)=='<' ? $text : htmlspecialchars($text, ENT_NOQUOTES|ENT_HTML5));
//		$html .= preg_match('/^[\s]*<.+>[\s]*$/su', $text)===1 ? $text : htmlspecialchars($text, ENT_NOQUOTES|ENT_SUBSTITUTE|ENT_HTML5, ini_get('default_charset'), false);
		$html .= 
			preg_match('/^[\s]*<.+>[\s]*$/su', $text)===1 ? 
			$text :
//			preg_match('/^[\r]?[\n]$/', $text)===1 ? $text : htmlspecialchars($text, ENT_NOQUOTES|ENT_SUBSTITUTE|ENT_HTML5, ini_get('default_charset'), false);
			preg_match('/^[\r]?[\n]$/', $text)===1 ? $text : html($text);
	return $html;
}
...
function _plain(...$texts) // 2018-08-11
{
	$plain = '';
	foreach ($texts as &$text) 
		$plain .= preg_match('/^[\s]*<.+>[\s]*$/su', $text)===1 ? $text : plain($text);
	return $plain;
}
